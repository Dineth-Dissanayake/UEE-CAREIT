...
const AllJobs = () => {
    ...
    useEffect(async () => {
        try {
            const res = await  fetch(`http://172.28.6.40:8050/PJobs`, {
                method: "GET"
            });
        ...
        return (
            <View key={index} style={styles.itemContainer}>
                <View style={styles.itemBody}>
                    ...
                    <Text style={styles.itemOthers}>Job Poster's Name: {item.jobpostername}</Text>
                    <Text style={styles.itemOthers}>Job Category: {item.jobcategory}</Text>
                    <Text style={styles.itemOthers}>Job type: {item.jobtype}</Text>
                    <Text style={styles.itemOthers}>email: {item.email}</Text>
                    <Text style={styles.itemOthers}>Mobile number: {item.mobilenumber}</Text>
                    <Text style={styles.itemOthers}>Location: {item.location}</Text>
                    <Text style={styles.itemOthers}>Salary: {item.salary}</Text>
